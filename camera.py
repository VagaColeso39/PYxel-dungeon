...
class Camera:
    def __init__(self, player:Player, level:Level, screen:pygame.display.Screen) -> None:
        self.cx = 0  # x of center of camera
        self.cy = 0  # y of center of camera
        self.to_x = 0
        self.to_y = 0
        self.strength = 5  # frames to push camera to target, that more that slower
    ...
    @property
    def tl_x(self):
        return self.cx - self.screen.get_surface().get_size()[0] // 2

    @property
    def tl_y(self):
        return self.cy - self.screen.get_surface().get_size()[1] // 2
