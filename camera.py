    ...
    def drawGrid(self):
            ...
            for y in range(0, self.level.level_height):
                ...
                if self.level.dungeon.grid[x][y].contains and self.player.is_visible(self.level.dungeon.grid, self.level.dungeon.grid[x][y]):
                    for item in self.level.dungeon.grid[x][y].contains:
                        ...
                        item.rect.x = int(x * self.block_size - self.tl_x)
                        item.rect.y = int(y * self.block_size - self.tl_y)
        ...
        for enemy in self.level.all_enemies:
            if self.player.is_visible(self.level.dungeon.grid, self.level.dungeon.grid[enemy.x][enemy.y]):
                ...
                enemy.rect.x = int(enemy.x * self.block_size - self.tl_x)
                enemy.rect.y = int(enemy.y * self.block_size - self.tl_y)
            else:
                enemy.visible = False
        ...
        self.player.rect.x = int(self.player.pos[0] * self.block_size - self.tl_x)
        self.player.rect.y = int(self.player.pos[1] * self.block_size - self.tl_y)
        self.draw_hud()
